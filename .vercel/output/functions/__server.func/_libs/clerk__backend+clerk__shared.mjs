...
//#endregion
//#region node_modules/@clerk/shared/dist/_chunks/error-CYyD2kei.mjs
/**
...
function parsePublishableKey(key, options = {}) {
	...
	if (!key || !isPublishableKey(key)) {
		if (options.fatal && !key) throw new Error(`Publishable key is missing. ${fatalKeyGuidance}`);
		if (options.fatal && !isPublishableKey(key)) throw new Error(`Publishable key not valid (expected format: pk_test_... or pk_live_...). ${fatalKeyGuidance}`);
		return null;
	}
...
//#endregion
//#region node_modules/@clerk/backend/dist/chunk-QOX5XVDR.mjs
...
var DEFAULT_CLOCK_SKEW_IN_MS = 5e3;
...
//#endregion
//#region node_modules/@clerk/backend/dist/chunk-RTBRJPLB.mjs
...
var AuthenticateContext = class {
	...
	*/
	...
	tokenBelongsToInstance(token) {
		...
		const { data, errors } = decodeJwt(token);
		if (errors) return false;
		if (typeof data.payload.iss !== "string") return false;
...
function getRemoteCache(scope) {
	...
	if (!cache) {
		for (const [key, entry] of remoteCaches) if (cacheHasExpired(entry)) remoteCaches.delete(key);
		cache = {
			keys: {},
			lastUpdatedAt: 0
		};
...
async function loadClerkJWKFromRemote(params) {
	...
	if (!jwk) throw new TokenVerificationError({
...
TokenType.ApiKey, TokenType.M2MToken, TokenType.OAuthToken;
...
var BillingSubscriptionItem = class _BillingSubscriptionItem {
	...
	static fromJSON(data) {
		...
		return new _BillingSubscriptionItem(data.id, data.instance_id, data.status, data.plan_period, data.period_start, nextPayment, data.amount ? billingMoneyAmountFromJSON(data.amount) : void 0, data.plan ? BillingPlan.fromJSON(data.plan) : null, data.plan_id ?? null, data.price_id ?? null, data.created_at, data.updated_at, data.period_end, data.canceled_at, data.past_due_at, data.ended_at, data.payer_id, data.is_free_trial, data.lifetime_paid ? billingMoneyAmountFromJSON(data.lifetime_paid) : void 0, data.seats ? billingSubscriptionItemSeatsFromJSON(data.seats) : void 0);
	}
};
var BillingSubscription = class _BillingSubscription {
	...
	static fromJSON(data) {
		...
		return new _BillingSubscription(data.id, data.instance_id, data.status, data.payer_id, data.created_at, data.updated_at, data.active_at ?? null, data.past_due_at ?? null, (data.subscription_items ?? []).map((item) => BillingSubscriptionItem.fromJSON(item)), nextPayment, data.eligible_for_free_trial ?? false);
	}
};
...
var EnterpriseConnectionCustomAttribute = class _EnterpriseConnectionCustomAttribute {
	...
	static fromJSON(data) {
		return new _EnterpriseConnectionCustomAttribute(data.name, data.key, data.sso_path, data.scim_path, data.multi_valued);
	}
};
...
var EnterpriseConnectionOauthConfig = class _EnterpriseConnectionOauthConfig {
	...
	static fromJSON(data) {
		return new _EnterpriseConnectionOauthConfig(data.id, data.name, data.client_id, data.discovery_url, data.logo_public_url, data.created_at, data.updated_at, data.provider_key, data.auth_url, data.token_url, data.user_info_url, data.requires_pkce);
	}
};
var EnterpriseConnection = class _EnterpriseConnection {
	...
	static fromJSON(data) {
		return new _EnterpriseConnection(data.id, data.name, data.domains, data.organization_id ?? null, data.active, data.sync_user_attributes, data.allow_subdomains, data.disable_additional_identifications, data.created_at, data.updated_at, data.saml_connection != null ? EnterpriseConnectionSamlConnection.fromJSON(data.saml_connection) : null, data.oauth_config != null ? EnterpriseConnectionOauthConfig.fromJSON(data.oauth_config) : null, data.provider, data.logo_public_url, data.allow_organization_account_linking, data.authenticatable, data.disable_jit_provisioning, data.custom_attributes?.map((attr) => EnterpriseConnectionCustomAttribute.fromJSON(attr)));
	}
};
...
async function verifyHandshakeJwt(token, { key }) {
	...
	assertHeaderAlgorithm(alg);
	if (isNonSessionJwtCategory(header.cat)) throw new TokenVerificationError({
		action: TokenVerificationErrorAction.EnsureClerkJWT,
		reason: TokenVerificationErrorReason.TokenInvalid,
...
function isTokenTypeInAcceptedArray(acceptsToken, authenticateContext) {
	...
	if (tokenInHeader) {
		if (isMachineToken(tokenInHeader)) parsedTokenType = getMachineTokenType(tokenInHeader);
...
var authenticateRequest = (async (request, options) => {
	...
	async function authenticateRequestWithTokenInHeader() {
		const { tokenInHeader } = authenticateContext;
		if (isMachineJwt(tokenInHeader) || hasNonSessionJwtCategory(tokenInHeader)) return signedOut({
			tokenType: TokenType.SessionToken,
			authenticateContext,
			reason: AuthErrorReason.TokenTypeMismatch,
			message: ""
	...
	async function authenticateRequestWithTokenInCookie() {
		...
		if (isMachineJwt(authenticateContext.sessionTokenInCookie) || hasNonSessionJwtCategory(authenticateContext.sessionTokenInCookie)) return signedOut({
			tokenType: TokenType.SessionToken,
			authenticateContext,
			reason: AuthErrorReason.TokenTypeMismatch,
			message: ""
...
//#endregion
export { reverificationError as A, PROD_API_URL as C, resolveAuthState as D, createCheckAuthorization as E, ClerkRuntimeError as F, getEnvVariable as M, buildErrorThrower as N, validateReverificationConfig as O, isClerkAPIResponseError as P, LOCAL_ENV_SUFFIXES as S, STAGING_ENV_SUFFIXES as T, isDevelopmentFromPublishableKey as _, debugRequestState as a, LEGACY_DEV_INSTANCE_SUFFIXES as b, isHttpOrHttps as c, proxyUrlToAbsoluteURL as d, retry as f, createDevOrStagingUrlCache as g, isDevelopmentEnvironment as h, createClerkRequest as i, isTruthy as j, isReverificationHint as k, isProxyUrlRelative as l, isAutomatedEnvironment as m, AuthStatus as n, deprecated as o, eventMethodCalled as p, constants as r, addClerkPrefix as s, createClerkClient as t, isValidProxyUrl as u, isDevelopmentFromSecretKey as v, STAGING_API_URL as w, LOCAL_API_URL as x, parsePublishableKey as y };
